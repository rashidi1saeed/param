...
def find_closest_op(ops, ts):
    ...
    closest_op = {}
    for op in ops:
        if has_category_field(op) and \
            get_timestamp_field(op) < ts and \
            (get_timestamp_field(op) + get_duration_field(op)) > ts and \
            get_timestamp_field(op) > closest_start:
...
def find_parent_cpu_op(kineto_gpu_op, kineto_et_ops, kineto_ac2g_s_ops,
    ...
    if kineto_gpu_op["args"]["External id"] in kineto_cpu_launcher_ops.keys():
        ts = get_timestamp_field(kineto_cpu_launcher_ops[kineto_gpu_op["args"]["External id"]]) + \
             get_duration_field(kineto_cpu_launcher_ops[kineto_gpu_op["args"]["External id"]])
        method = "cuda launch"
    # Find an arrow (CPU->GPU) start point which has the same external id with the GPU op.
        ...
        ts = get_timestamp_field(kineto_ac2g_s_ops[kineto_gpu_op["args"]["External id"]])
        method = "ac2g_s"
    # Find an arrow (CPU->GPU) final destination point which has the same external id with the GPU op.
        ...
        ts = get_timestamp_field(kineto_ac2g_f_ops[kineto_gpu_op["args"]["External id"]])
        method = "ac2g_f"
    assert ts != -1
    ...
    if not parent_cpu_op:
        print("Warning! the parent cpu_op for the following gpu_op is not found" \
                + "and hence it is discarded. gpu_op name: " + str(kineto_gpu_op["name"]) \
                + ", ts: " + str(get_timestamp_field(kineto_gpu_op)) \
                + ", external_id: " + str(kineto_gpu_op["args"]["External id"]))

    #TODO: In some cases, condition 2 is not met. Seems to be a problem in input from older Kineto.
    #assert parent_cpu_op and get_timestamp_field(parent_cpu_op) + get_duration_field(parent_cpu_op) > get_timestamp_field(kineto_gpu_op)
...
def trace_analysis(et_file, kineto_file, annotation="DataLoader"):
    ...
    sorted_kineto_trace_ops = sorted(kineto_trace_ops, key=lambda kv: get_timestamp_field(kv))

    kineto_et_ops = [
        op
        for op in sorted_kineto_trace_ops
        if has_category_field(op) and "ProfilerStep" not in get_name_field(op)
        and (get_category_field(op) == "cpu_op" or get_category_field(op) == "user_annotation")
    ]
    ...
    kineto_ac2g_s_ops = {
        ...
        for op in sorted_kineto_trace_ops
        if has_category_field(op) and "ProfilerStep" not in get_name_field(op)
        and (get_category_field(op) == "ac2g" and get_phase_field(op) == "s")
    }

    kineto_ac2g_f_ops = {
        op["id"]:op
        for op in sorted_kineto_trace_ops
        if has_category_field(op) and "ProfilerStep" not in op["name"]
        and (get_category_field(op) == "ac2g" and get_phase_field(op) == "f")
    }

    kineto_cpu_launcher_ops = {
        op["args"]["External id"]:op
        for op in sorted_kineto_trace_ops
        if has_category_field(op)
        and (get_category_field(op) == "cuda_runtime" \
        and (get_name_field(op) == "cudaLaunchKernel" or get_name_field(op) == "cudaMemcpyAsync"))
    }
    ...
    kineto_gpu_ops = [
        ...
        for op in sorted_kineto_trace_ops
        if has_category_field(op) and "ProfilerStep" not in get_name_field(op)
        and (get_category_field(op) == "kernel" or get_category_field(op) == "gpu_memcpy")
    ]
    ...
    return et_nodes, kineto_et_ops, kineto_ac2g_s_ops, kineto_ac2g_f_ops,\
            kineto_cpu_launcher_ops, kineto_gpu_ops
...
def op_exists(name, kineto_et_ops, i):
    ...
    while distance <= MAX_DISTANCE:
        ...
        if forward_index < len(kineto_et_ops) \
        ...
        if backward_index >= 0 and get_name_field(kineto_et_ops[backward_index]) == name:
...
def exact_match(kineto_et_ops, kineto_ac2g_s_ops, kineto_ac2g_f_ops,
    ...
    for i in range(len(kineto_et_ops)):
        ...
        if op["tid"] not in kineto_op_per_thread:
            ...
            kineto_op_per_thread[op["tid"]]["end_ts"] = get_timestamp_field(op) + get_duration_field(op)
        ...
        process_end_time = max(process_end_time, get_timestamp_field(op) + get_duration_field(op))
    ...
    for gpu_op in kineto_gpu_ops:
        parent_cpu_op = find_parent_cpu_op(gpu_op, kineto_et_ops, kineto_ac2g_s_ops, \
                kineto_ac2g_f_ops, kineto_cpu_launcher_ops)
    ...
    if len(kineto_et_ops) >= len(et_nodes):
        for i in range(len(et_nodes)):
            ...
            if (name_exist or
                ...
                et_node.name.replace("execution_graph", "execution_trace") == get_name_field(kineto_et_op)):
                ...
                if "args" in kineto_et_op and "Ev Idx" in kineto_et_op["args"] \
                ...
                logger.info("Op mismatch between kineto and execution trace ( et size = " \
                        + str(len(et_nodes))+", kineto size: "+str(len(kineto_et_ops))+" ):")
                logger.info(
                ...
                for i in range(len(kineto_et_ops)):
                    ...
                    et_node = et_nodes[i]
                    logger.info( "Index: "+str(i)+", et name: " + et_node.name \
                            + ", kineto name: " + get_name_field(kineto_et_op))

                exit(0)
    else:
        logger.info("Ops count mismatch between kineto and execution trace ( et size = " \
                + str(len(et_nodes))+", kineto size: "+str(len(kineto_et_ops))+" )")
    ...
    for thread in kineto_op_per_thread:
        ...
        for op in kineto_op_per_thread[thread]:
            if get_timestamp_field(op) < kineto_nodes[-1].end:
                tmp = Kineto_node(
                    get_name_field(op), get_timestamp_field(op), get_timestamp_field(op) + get_duration_field(op), cnt
                )
                ...
                while kineto_nodes[-1].end <= get_timestamp_field(op) and len(kineto_nodes) > 1:
                    kineto_nodes.pop()
                tmp = Kineto_node(
                    get_name_field(op), get_timestamp_field(op), get_timestamp_field(op) + get_duration_field(op), cnt
                )
...
# Assign unique ET id by checking assigned ids
    ...
    with open(args.et_file, "r") as f:
        ...
        for node in et["nodes"]:
            ...
            if orig_node_id in et_gpu_ops_per_cpu_op_id:
                gpu_nodes = update_gpu_nodes(et_gpu_ops_per_cpu_op_id, node,
...
if __name__ == "__main__":
    ...
    et_gpu_ops_per_cpu_op_id = {}
    et_nodes, kineto_et_ops, kineto_ac2g_s_ops, kineto_ac2g_f_ops, kineto_cpu_launcher_ops, kineto_gpu_ops = \
        trace_analysis(args.et_file, args.kineto_file, args.annotation)

    if args.exact_match:
        et_enhanced_duration, et_enhanced_timestamp, et_gpu_ops_per_cpu_op_id = \
            exact_match(kineto_et_ops, kineto_ac2g_s_ops, kineto_ac2g_f_ops, \
            kineto_cpu_launcher_ops, kineto_gpu_ops, et_nodes)
        # If linking works, add duration time to each ET node and dump as ET_plus
        dump_et_file(et_enhanced_duration, et_enhanced_timestamp, et_gpu_ops_per_cpu_op_id)
    else:
        logger.info("This script works only with the exact match mode," \
