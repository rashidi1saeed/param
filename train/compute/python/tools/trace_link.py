...
def trace_analysis(et_file, kineto_file, annotation="DataLoader"):
    ...
    return (et_nodes, kineto_et_events, kineto_ac2g_s_events, kineto_ac2g_f_events,
            kineto_cpu_launch_kernel_events, kineto_gpu_events)
...
def find_parent_cpu_op(
    ...
    kineto_ac2g_f_events,
    kineto_cpu_launch_kernel_events):
    external_id = kineto_gpu_event["args"]["External id"]

    if external_id in kineto_cpu_launch_kernel_events:
        ts = (kineto_cpu_launch_kernel_events[external_id]["ts"]
              + kineto_cpu_launch_kernel_events[external_id]["dur"])
    elif external_id in kineto_ac2g_s_events:
        ts = kineto_ac2g_s_events[external_id]["ts"]
    elif external_id in kineto_ac2g_f_events:
        ts = kineto_ac2g_f_events[external_id]["ts"]
    else:
        assert False, "Timestamp not set."
    ...
    parent_cpu_op = {}
    for event in kineto_et_events:
        if ("cat" in event and event["ts"] < kineto_gpu_event["ts"]
           and (event["ts"] + event["dur"]) > kineto_gpu_event["ts"]
           and event["ts"] > closest_start):
...
def exist(name, kineto_et_events, i):
    MAX_DISTANCE = 0
    distance = 0
    while ((distance <= MAX_DISTANCE) and (distance + i < len(kineto_et_events)))\
            or (i - distance >= 0):
        if (distance + i < len(kineto_et_events)
           and name == kineto_et_events[distance + i]["name"]):
            return True, kineto_et_events[distance + i]
        elif (i - distance >= 0
              and name == kineto_et_events[i - distance]["name"]):
        ...
        for i in range(len(et_nodes)):
            ...
            if (
                ...
                if ("args" in kineto_et_event) and ("Ev Idx" in kineto_et_event["args"])\
                        and (kineto_et_event["args"]["Ev Idx"] in gpu_kernels_per_cpu_event_idx):
...
def assign_ids(total_assigned_ids, assigned_ids, id):
    ...
    while True:
        if id in total_assigned_ids:
            ...
            if orig_id not in assigned_ids:
...
if __name__ == "__main__":
    ...
    gpu_kernels_per_cpu_event_id = {}
    et_nodes, kineto_et_events, kineto_ac2g_s_events, kineto_ac2g_f_events,\
            kineto_cpu_launch_kernel_events, kineto_gpu_events\
            = trace_analysis(args.et_file, args.kineto_file, args.annotation)

    if args.exact_match:
        et_enhanced_duration, et_enhanced_timestamp, gpu_kernels_per_cpu_event_id\
                = exact_match(kineto_et_events, kineto_ac2g_s_events, kineto_ac2g_f_events,
                              kineto_cpu_launch_kernel_events, kineto_gpu_events, et_nodes)
